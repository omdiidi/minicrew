...
for i in $(seq 1 "$WORKERS"); do
  if [ -n "$POLL_INTERVAL" ]; then
    .venv/bin/python -m worker.utils.launchd install --instance "$i" --role "$ROLE" --config-path "$MINICREW_CONFIG_PATH" --poll-interval "$POLL_INTERVAL"
  else
    .venv/bin/python -m worker.utils.launchd install --instance "$i" --role "$ROLE" --config-path "$MINICREW_CONFIG_PATH"
  fi
done
